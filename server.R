...
server <- function(input, output) {
  ...
  output$descriptives <- renderUI({
    ...
    if(input$transform=="Add Outlier"){
      ...
      y_m<-y_n[2:length(y_n)]
      y<-c(29,y_m)
    } else if (input$transform=="Add 5 points to each score"){
      y=data()+5
    } else if (input$transform=="Multiply all scores by 2"){
      y=2*data()
    } else if (input$transform=="Divide all scores by 2"){
      y=data()/2
    } else if (input$transform=="Calculate z-scores"){
      y=(data()-mean(data()))/sd(data())
    }
